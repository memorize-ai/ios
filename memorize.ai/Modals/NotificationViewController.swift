...
class NotificationViewController: UIViewController {
	...
	func show(_ delay: TimeInterval, completion: (() -> Void)?) {
		...
		UIView.animate(withDuration: 0.125, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: .curveEaseOut, animations: {
			self.notificationView.transform = .identity
		}) {
			guard $0 else { return }
			Timer.scheduledTimer(withTimeInterval: delay, repeats: false) { _ in
				UIView.animate(withDuration: 0.125, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: .curveEaseIn, animations: {
